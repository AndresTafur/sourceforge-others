...
whSQL::SQLResult whSQL::SQLiteSession::getResultAt(unsigned int index)
{
...
void whSQL::SQLiteSession::ereaseValueAt(unsigned int index)
{
    if( index == 0)
        results.pop_front();
    else if( index == (results.size()-1))
...
void whSQL::SQLiteSession::doQuery(std::string query)
{
    if( results.size() > 0)
        results.clear();
    if( !query.empty())
