...
public:
	void createRagDoll()
    {
		...
		ragdoll->getAnimationState(meshZombieAnimation[sSelectedMesh])->setEnabled(true);
...
#endif //OGRE_VERSION not heihort
	{
        ...
        {   
            ...
            {
                (*zE)->getAnimationState(meshZombieAnimation[b->sSelectedMesh])->addTime(time * _animation_speed);
        ...
        {  
                ...
                ++zE)
            {
                if ((*zE)->isPhysical())
                {
                    ...
                    ragDollData *b = any_cast<ragDollData *> ((*zE)->getUserAny());
                    (*zE)->getAnimationState(meshZombieAnimation[b->sSelectedMesh])->setEnabled(true);
                }
            }
...
//#define _DEBUG_ZOMBIE_TRIMESH
					...
					if (!wasPhysical)
					{
						...
						radgoll->getAnimationState(meshZombieAnimation[b->sSelectedMesh])->setEnabled(false);
					...
					if (!is_hit&& !wasPhysical)
					{
						...
						ragDollData *b = any_cast<ragDollData *> (radgoll->getUserAny());
						radgoll->getAnimationState(meshZombieAnimation[b->sSelectedMesh])->setEnabled(true);
					}
#endif //_TESTTRIMESH	
