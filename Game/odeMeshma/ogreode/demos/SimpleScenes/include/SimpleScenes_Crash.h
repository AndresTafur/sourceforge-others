...
public:
    ...
    virtual void frameEnded(Real time, OIS::Keyboard* input, OIS::Mouse* mouse)
    {
		...
		// Throttle up or down
	if (input->isKeyDown(OIS::KC_I)) _main_spd += (_main_response * time);
	else if (input->isKeyDown(OIS::KC_K)) _main_spd -= (_main_response * time);

		// Thrust left, right, forward, or back
		_thrust_force = Ogre::Vector3::ZERO;
		if (input->isKeyDown(OIS::KC_G)) _thrust_force.z += _thrust;
		if (input->isKeyDown(OIS::KC_B)) _thrust_force.z -= _thrust;
		if (input->isKeyDown(OIS::KC_V)) _thrust_force.x += _thrust;
		if (input->isKeyDown(OIS::KC_N)) _thrust_force.x -= _thrust;
		...
		if (input->isKeyDown(OIS::KC_J)) _tail_spd += (_tail_response * time);
		else if (input->isKeyDown(OIS::KC_L)) _tail_spd -= (_tail_response * time);
		else
		{
			if (_tail_spd < 0.0)
			{
