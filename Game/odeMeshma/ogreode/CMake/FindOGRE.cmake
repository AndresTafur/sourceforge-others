...
find_path(OGRE_HOME NAMES OgreMain/include/Ogre.h include/Ogre.h HINTS ${OGRE_PATHS} PATH_SUFFIXES OGRE)
SET(OGRE_SOURCE ${OGRE_HOME})
IF("${OGRE_HOME}" STREQUAL "OGRE_HOME-NOTFOUND")
# Warn user that the Ogre path was not found.
	MESSAGE("WARNING: OGRE source path not found.  To ensure the source files are included properly, please set OGRE_HOME in the system environment or in CMake to the correct path.")
ELSE()
	PKG_MESSAGE(OGRE "OGRE source path set to ${OGRE_HOME}.  To set this manually, set OGRE_HOME in CMake to the desired path.")
ENDIF()

# Find the Ogre build directory if the Ogre home directory has changed.
PKG_MESSAGE(OGRE "Now searching for path to Ogre build files.")
SET(OGRE_BUILD OGRE_BUILD-NOTFOUND)
find_path(OGRE_BUILD NAMES bin/Release/OgreMain.dll HINTS ${OGRE_HOME}/build ${OGRE_HOME} PATH_SUFFIXES OGRE)
IF("${OGRE_BUILD}" STREQUAL "OGRE_BUILD-NOTFOUND")
	MESSAGE("Could not find Ogre build files.  Please set OGRE_BUILD to the correct path in CMake.")
ELSE()
	PKG_MESSAGE(OGRE "OGRE build files found at ${OGRE_BUILD}.")
ENDIF()
